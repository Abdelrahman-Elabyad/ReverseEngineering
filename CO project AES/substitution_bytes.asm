...
sbox db 0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F, 0xC5, 0x30, 0x01, 0x67, 0x2B, 0xFE, 0xD7, 0xAB, 0x76
     ...
     db 0x53, 0xD1, 0x00, 0xED, 0x20, 0xFC, 0xB1, 0x5B, 0x6A, 0xCB, 0xBE, 0x39, 0x4A, 0x4C, 0x58, 0xCC
...
data db 0x31
matrix  db 0x63, 0x7C, 0x77, 0x7B
        db 0xCA, 0x82, 0xC9, 0x7D 
        db 0xB7, 0xFD, 0x93, 0x26
        db 0x04, 0xC7, 0x23, 0xC3
...
loop_start:
    mov ch,0
    mov cl, [counter]       ; Load the current counter value into CL
    lea si, matrix          ; Load the base address of the matrix into SI
    add si, cx              ; Add counter (CL) to SI to calculate the effective address
    mov al, [si]            ; Load the value from the matrix into AL

    lea si, sbox            ; Load the base address of the sbox into SI
    add si, ax              ; Add BL (low byte of BX) to SI for the sbox index
    mov al, [si]            ; Load the corresponding value from sbox into AL

    lea si, matrix          ; Reload the base address of the matrix into SI
    add si, cx              ; Recalculate the matrix address for storing
    mov [si], al            ; Store the updated value back into the matrix
